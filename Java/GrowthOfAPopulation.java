...
class GrowthOfAPopulation {
  ...
  public static int nbYear(int p0, double percent, int aug, int p) {
    return nbYear(p0, percent, aug, p, 0);
  }

  //return the year when p0 >= p,
  //otherwise calc the newP0 and increment year.
  private static int nbYear(int p0, double percent, int aug, int p, int year) {
    if (p0 >= p) {
      return year;
    } else {
      int newP0 = (int) (p0 + p0 * (percent / 100) + aug);
      return nbYear(newP0, percent, aug, p, year + 1);
    }
  }
}
